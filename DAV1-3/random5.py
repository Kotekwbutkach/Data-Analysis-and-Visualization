...
def create_plots(df, country_codes, output_path: str, show=False, codes=False, color: str = "def"):
    df.sort_values(df.columns[-1], ascending=False, inplace=True)
    rind = random.randrange(0, len(df.index))
    rcol = random.randrange(0, len(df.columns))
    df1 = df.loc[df_closest_entries(df, rind, df.columns[rcol], 5)]
    suptitle = f"Population in years {df.columns[0]}-{df.columns[-1]}"
    title = f"5 countries with {df.index[rind]} as centroid for year {df.columns[rcol]}"
    df1.sort_values(df.columns[-1], ascending=False, inplace=True)
...
if __name__ == '__main__':
    ...
    anim_plot = create_plots(df, country_codes, 'output/graphgif_random5.gif', show=True)
